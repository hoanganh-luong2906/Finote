...
const FinoteAccordion = ({ dark }: any) => {
	const accordionData = [
		{
			id: 1,
			title: 'How can I send money with Finote?',
			desc: 'Discover the step-by-step process to make money transfers easily and securely with Finote. <br /> <br /> When I hear the buzz of the little world among the stalks, and grow familiar with the countless indescribable forms of the insects and flies, then I feel the presence of the Almighty, who formed us in his own image, and the breath',
		},
		{
			id: 2,
			title: 'What security measures does Finote have to protect my transactions?',
		...
		{
			id: 3,
			title: 'What is the coverage of the Finote network?',
			desc: 'Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.',
		},
		{
			id: 4,
			title: 'How can I contact Finote customer service?',
	...
	return (
		<div className={`mil-accordion`}>
			{accordionData.map((item) => (
				<div
					className={`mil-accordion-group mil-up ${active == item.id ? ' mil-active' : ''}`}
					key={item.id}
				>
					<div
					...
					<div className='mil-accordion-content'>
						<p className='mil-text-m mil-soft' dangerouslySetInnerHTML={{ __html: item.desc }}></p>
					</div>
				</div>
			))}
		</div>
...
export default FinoteAccordion;
